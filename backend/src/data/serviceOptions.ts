export const CATALOG_OPTION_GROUPS = ['language', 'delivery-mode', 'audience', 'offer-type', 'availability'] as const
export type CatalogOptionGroup = (typeof CATALOG_OPTION_GROUPS)[number]

export type ServiceOptionSeed = {
...
export const SERVICE_OPTIONS_SEED: readonly ServiceOptionSeed[] = [
...
export function serviceOptionValue(option: Pick<ServiceOptionSeed, 'group' | 'slug' | 'name'>) {
...
export function serviceOptionValues(group: CatalogOptionGroup) {
  return SERVICE_OPTIONS_SEED.filter((item) => item.group === group).map(serviceOptionValue)
}
